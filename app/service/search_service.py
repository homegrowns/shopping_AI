...
def get_text_vector(msg: str, embedder):
    ...
    """
    # find 결과가 -1이 아닌 첫 번째 값을 선택
    index = next(
        (
            i
            for i in [msg.find(" that"), msg.find(" matching"), msg.find(" 추천")]
            if i != -1
        ),
        -1,
    )
...
def get_image_vector(
    ...
    box: Optional[
        List[int]
    ],  # [x1, y1, x2, y2] 형태의 리스트나 튜플 (없을 수도 있으니 Optional)
    contents: Optional[
        Any
    ],  # 추출된 텍스트나 결과값 (타입에 맞춰 str 등으로 변경 가능)
...
def build_query_vector(
    ...
    # 영어가 아닌경우 번역
        ...
        try:
            ...
            text_vector = get_text_vector(msg, embedder)
            print(f"[MSG] {message} [translation] {msg}\n")
        except Exception as e:
            print(f"[ERROR] 번역 실패: {e}")
            msg = message

    image_vector, crop_applied = get_image_vector(
        original_image, box, contents, embedder
    )
    ...
    if image_vector and text_vector:
        ...
        if any(k in msg for k in KEYWORD):  # 예} 이 바지 보다 밝은
            ...
            print(
                f"텍스트와[{TEXT_WEIGHT}] 이미지[{IMAGE_WEIGHT}]의 가중합으로 멀티모달 쿼리 벡터 생성.\n\n"
            )

            # 이미지 벡터에 0.4, 텍스트 벡터에 0.6을 곱해서 더해줍니다.
            combined = (IMAGE_WEIGHT * np.array(image_vector)) + (
                TEXT_WEIGHT * np.array(text_vector)
            )
    ...
    return query_vector, crop_applied, is_image_collection
