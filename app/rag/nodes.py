...
def chatbot(state: AgentState):
    ...
    writer({"type": "status", "status": "🦾 질문 분석 중..."})
...
def qdrant_search(state: AgentState):
    ...
    writer({"type": "status", "status": "👀 상품 검색 중..."})
...
def context_organizer(state: AgentState):
    ...
    writer({"type": "status", "status": "🔥상품 요약중🔥..."})
        ...
        rag_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    ...
                    사용자가 원하는 정확한 상품을 찾지 못한 상황입니다. 사용자에게 정중히 양해를 구하고 
                    열심히 찾아보고 있다고 말하세요 """,
                ),
                (
                    "user",
